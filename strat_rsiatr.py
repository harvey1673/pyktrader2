...
class RsiAtrStrat(Strategy):
    ...
    def update_mkt_state(self, idx):
        ...
        atrma_key = "MA_A" + str(self.atrma_win[idx])
        self.atrma[idx] = xdf[atrma_key][-1]

    def on_bar(self, idx, freq):
        self.update_mkt_state(idx)
        if (freq == self.freq[idx]) and self.check_trigger(idx):
            self.save_state()
    ...
    def check_trigger(self, idx):
        ...
        if buysell != 0:
            msg = 'RSI-ATR to open position for inst = %s, ATRMA=%s, ATR=%s, RSI=%s, curr_price= %s, direction=%s, volume=%s' \
                                % (inst, self.atrma[idx], self.atr[idx], self.rsi[idx], curr_p, buysell, self.trade_unit[idx])
            self.open_tradepos(idx, buysell, curr_p + buysell * tick_base)
            self.status_notifier(msg)
            save_status = True
        return save_status
